	...
	$($(".hot-l:visible img.camera").get(index)).fadeIn();
      ...
      $("input.val").blur(function(){
      	if ($(this).val().length == 0) {
      		$(this).siblings(".tishi").show();
      		$(this).addClass("error");
      		$(this).siblings(".tishi").addClass("error").text("填写内容")
      	} else {
      		$(this).siblings(".tishi").hide();
      		$(this).removeClass("error");
      		$(this).siblings(".tishi").removeClass("error");
      ...
      var num = $("div.pic img.big").length;
      if (num > 1) {
      	for (var i=0; i<num;i++){
      		var li = $("<li><a href =\"javascript:;\"></a></li>");
      		$("div.pic ul").append(li);
      	}
      	$("div.pic ul li:nth-child(1)").addClass("highlight");
      	$(".pic li").click(function(){
      		var index = $(".pic li").index($(this));
      		var img = $(".pic img.big");
      		$(".pic img.big").hide();
      		$(img.get(index)).show();
      		$(".pic li").removeClass("highlight");
      ...
      var divNum = $(".hot-l").length;
      for (var i = 0; i < divNum; i++){
      	var curDiv = $($(".hot-l").get(i));
      	var imgNum = curDiv.find("img.camera").length;
      	if (imgNum > 1) {
      		for(var j = 0; j < imgNum; j++){
      			var li = $("<li><a href=\"javascript:;\"></a></li>");
      			curDiv.find("ul").append(li);
      		}
      		curDiv.find("ul li:nth-child(1)").addClass("highlight");
      		$(".hot-l li").click(function(){
      			clearInterval(interval);
      			var index = $(".hot-l:visible li").index($(this));
      			var img = $(".hot-l:visible img:camers");
      			$(".hot-l:visible li").removeClass("highlight");
      			$(this).addClass("highlight");
      			$(this).addClass("highlight");
      			interval = setInterval(changeIconBanner,4000);
      ...
      $("td.delect-botton").click(function(){
      	var allTableRow = $("tr.table-row");
      	selectedRow = allTableRow.index($(this).parents(".table-row"));
      });


      $("button.yes").click(function(){
      	$($("tr.table-row").get(selectedRow)).remove();
      	$(".delete-k").parents(".overlay").hide();
      	$(".delect-k").hide();
      });
      ...
      $(".like-list").hover(function(){
      	$(this).find("p.like-bottom").slideDown();
      	$(this).find("a.dian").animate({margin:"0px"},300);
      	$(this).find("a.dian").css("color","#8fc31f");
      },function(){
      	$(this).find("p.like-bottom").slideUp();
      	$(this).find("a.dian").animate({margin:"10px 0px 5px"},300);
      	$(this).find("a.dian").css("color","#646464");
      });
      ...
      $("trigger-wrap li").click(function(){
      	$(".trigger-wrap li").removeClass("highlight");
      	$(this).addClass("highlight");
      	var index = $(".trigger-wrap li.highlight:visible").data("index");
      	$(".panel").hide();
      ...
      $("a.plus").click(function(){
      	...
      	var price = parseFloat($(this).parents("re.table-row").find("td:nth-child(6)").text());
      	var totalPrice = price*num;
      	$(this).parents("tr.table-row").find("td:nth-child(6)").text(totalPrice);
      	$(this).parents("tr.table-row").find("td:nth-child(8) span").text(totalPrice);

      	var input = $(this).parents("tr.rable-row").find("input");
      	if(!input.is(":checked")){
      		input.attr("checked","checked");
      		input.change();
      	}
      ...
      $(".item-r").hover(function(){
      	...
      	$(this).find("a.x-button").animate({bottom:"33px"},300);
      });
