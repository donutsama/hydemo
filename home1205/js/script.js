$(function(){
	...
	// 	// $(this).addClass("highlight");
	// 	$(".indicator li").removeClass("highlight");   //li去掉高亮效果
	// 	$(this).addClass("highlight");                 //给当前li加高亮
	// 	var index = $(".indicator li").index($(this));   //获取当前这个li 在这个排在第几个
	// 	picBannerIndex = index;
	// 	var item = $(".pic-banner .pic-item[data-index='" + (index+1) + "']");    //获取pic-item为index+1
	...
	$(".xuan").hover(function(){               //鼠标移动上去显示购物车（积分专区）
		$(this).children("span").css("border-bottom","1px solid #fafafa");
		$(this).find("#qq").show();
